import type { VeloceRequest, VeloceResponse, VeloceVehicleResult, ResolvedRoute } from '@/types/veloce'
...
export function buildVelocePayload(
  ...
  const stops = deliveries
    .filter((d) => d.status === 'pending' || d.status === 'preassigned' || d.status === 'assigned' || d.status === 'at_risk')
    .map((d) => ({
      id: d.id,
...
export function applyVeloceSolution(
  ...
  const updatedDeliveries = deliveries.map((d) => {
    ...
    // Preserve user-set "preassigned" — it's a manual lock that the optimizer respects.
    if (d.status === 'preassigned') return d
